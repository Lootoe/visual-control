import { getFaceData } from './getFaceData'
...
export const unifyNormal = (polygon, points) => {
  const faceData = getFaceData(polygon, points)
  const correctPolygon = []

  for (const face of faceData) {
    // 经测试，0.5这个值是最理想的
    const tempPoint = getPointOnNormal(face, 0.5)
    if (isPointInPolygon(tempPoint, faceData, points)) {
      correctPolygon.push([face.vertices[2], face.vertices[1], face.vertices[0]])
    } else {
